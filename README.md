# Ex08 CAMU Schedule using Bootstrap
## Date: 30/03/26

## AIM:
To design a responsive and visually appealing CAMU Schedule using Bootstrap.

## DESIGN STEPS:

### Step 1:
Clone the repository from GitHub.

### Step 2:
Create Django Admin project.

### Step 3:
Create a New App under the Django Admin project.

### Step 4:
Add the Bootstrap CDN link inside the ```<head>``` section.

### Step 5:
Insert a table element with Bootstrap table classes.

### Step 6:
Construct the complete table.

### Step 7:
Add a header/footer displaying copyright information.

### Step 8:
Publish the website in the LocalHost.

## PROGRAM :
```
<html>
<head>
    <title>SLOT TIME TABLE</title>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.7.1/jquery.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>

</head>

<body bgcolor="#f0f8ff">

    <center>
        <img src="logo.png" width="600" height="120" title="Saveetha Engineering College">
    </center>

    <br><br>
    <table border="3" cellpadding="12" cellspacing="5" width="80%" align="center" bgcolor="#e6f2ff">
        
        <tr bgcolor="#4da6ff">
            <th>Timing</th>
            <th>Monday</th>
            <th>Tuesday</th>
            <th>Wednesday</th>
            <th>Thursday</th>
            <th>Friday</th>
            <th>Saturday</th>
        </tr>

        <tr align="center">
            <td bgcolor="#99ccff">8 - 10</td>
            <td>Web</td>
            <td>Web</td>
            <td>Web</td>
            <td>Free</td>
            <td>Web</td>
            <td>ML</td>
        </tr>

        <tr align="center">
            <td bgcolor="#99ccff">10 - 12</td>
            <td>C</td>
            <td>C</td>
            <td>ML</td>
            <td>Free</td>
            <td>C</td>
            <td>Web</td>
        </tr>

        <tr align="center">
            <td bgcolor="#99ccff">12 - 1</td>
            <td colspan="6" bgcolor="#ffd699"><b>Lunch Break</b></td>
        </tr>

        <tr align="center">
            <td bgcolor="#99ccff">1 - 3</td>
            <td>ML</td>
            <td>ML</td>
            <td>Mentor Meet</td>
            <td>C</td>
            <td>ML</td>
            <td>C</td>
        </tr>

    </table>

    <br><br>
    <table border="3" cellpadding="10" cellspacing="5" width="50%" align="center" bgcolor="#fff0f5">

        <tr bgcolor="#ff66a3">
            <th>S.No</th>
            <th>Subject Code</th>
            <th>Subject Name</th>
        </tr>

        <tr align="center">
            <td>1</td>
            <td>19AI414</td>
            <td>Web Application</td>
        </tr>

        <tr align="center">
            <td>2</td>
            <td>19AI304</td>
            <td>C Programming</td>
        </tr>

        <tr align="center">
            <td>3</td>
            <td>19AI410</td>
            <td>Machine Learning</td>
        </tr>

    </table>

</body>
</html>

```

## OUTPUT:

![alt text](image-1.png)

## RESULT:
A responsive and visually appealing CAMU Schedule web page using Bootstrap is designed successfully.
